...
Note that `StaticPolynomials` objects cannot be directly generated from strings. So first a Julia function is generated by parsing and evaluating
the strings in `cf8` containing the polynomial system. Using this function a `DynamicPolynomials` object is generated which is then tranformed
to a `StaticPolynomials` object.
```julia
